...
static volatile uint32_t pulse_count_test = 0;
...
FanSpeeds getAllFanSpeeds() {
    ...
    for (uint8_t fanIndex = 0; fanIndex < 4; fanIndex++) {
        if (current_time - last_time_test[fanIndex] >= 1000) {  // Calculate RPM every second
            uint32_t pulses_test = pulse_count[fanIndex] - last_pulse_count_test[fanIndex];
...
uint32_t getRPM(){
    ...
    if (current_time - last_time >= 1000) {  // Calculate RPM every second
        uint32_t pulses = pulse_count_test - last_pulse_count;
        last_pulse_count = pulse_count_test;
